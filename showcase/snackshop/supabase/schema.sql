...
-- Nội dung giới thiệu, thông tin liên hệ và mạng xã hội hiển thị ở trang chủ/footer
alter table public.site_settings add column if not exists about_title text not null default 'Giới thiệu';
alter table public.site_settings add column if not exists about_text text not null default 'Đồ ăn vặt ngon mỗi ngày, được chọn lọc kỹ từ hương vị đến chất lượng.';
alter table public.site_settings add column if not exists footer_description text not null default 'Đồ ăn vặt online — giao tận nơi.';

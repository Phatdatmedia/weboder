...
document.getElementById("search-input").addEventListener("keydown", (e) => {
  ...
  async function render() {
    if (!slug) { root.innerHTML = `<div class="empty-state"><h3>Không tìm thấy sản phẩm</h3></div>`; return; }
    try {
      ...
      root.innerHTML = "";
      root.appendChild(el("div", { class: "pd-image" }, el("img", { src: p.image_url || "https://placehold.co/600x600/191C26/57E8CB?text=PC.STORE", alt: p.name })));

      const info = el("div", {}, [
        el("div", { class: "product-cat" }, p.categories?.name || ""),
        el("h1", { class: "pd-title" }, p.name),
        el("div", { class: "pd-price mono" }, [
          formatVND(price),
          p.sale_price ? el("span", { class: "product-price--old mono", style: "margin-left:10px;" }, formatVND(p.price)) : null,
        ]),
        el("div", { class: "stock-tag" + (p.stock <= 0 ? " stock-tag--out" : "") }, p.stock > 0 ? `Còn ${p.stock} sản phẩm trong kho` : "Tạm hết hàng"),
      ]);
      ...
      if (p.description) info.appendChild(el("p", { style: "color:var(--muted);" }, p.description));

      const stepper = el("div", { class: "qty-stepper" }, [
        el("button", { onclick: () => { qtyInput.value = Math.max(1, +qtyInput.value - 1); } }, "−"),
        (function () { const i = document.createElement("input"); i.value = 1; i.id = "qty-input"; return i; })(),
        el("button", { onclick: () => { qtyInput.value = Math.min(p.stock, +qtyInput.value + 1); } }, "+"),
      ]);
      info.appendChild(el("div", { style: "margin:18px 0;" }, stepper));
      ...
      const addBtn = el("button", {
        ...
        onclick: async () => {
          ...
          try {
            await addToCart(user.id, p.id, Math.max(1, +qtyInput.value || 1));
      ...
      root.innerHTML = `<div class="empty-state"><h3>Không tải được sản phẩm</h3><p>${e.message}</p></div>`;
    }
  }

...
document.getElementById("search-input").addEventListener("keydown", (e) => {
    if (e.key === "Enter") window.location.href = `/showcase/pcstore-single/?q=${encodeURIComponent(e.target.value)}`;
  });
  ...
  async function refreshCartCount() {
    document.getElementById("cart-count").textContent = await getCartCount(user?.id);
  }

  function cartRow(item) {
    const p = item.products;
    const row = el("div", { class: "cart-row" }, [
      el("img", { src: p.image_url || "https://placehold.co/100x100/191C26/57E8CB?text=PC", alt: p.name }),
      el("div", {}, [
        el("div", { class: "cart-thumb-name" }, p.name),
        el("div", { class: "mono", style: "color:var(--muted); font-size:.8rem;" }, formatVND(p.sale_price ?? p.price)),
      ]),
      (function () {
        const stepper = el("div", { class: "qty-stepper" });
        const minus = el("button", {}, "−");
        const input = document.createElement("input"); input.value = item.quantity;
        const plus = el("button", {}, "+");
        minus.onclick = async () => { await updateCartQuantity(item.id, +input.value - 1); render(); };
        plus.onclick = async () => { await updateCartQuantity(item.id, +input.value + 1); render(); };
        stepper.append(minus, input, plus);
        return stepper;
      })(),
      el("div", { class: "cart-line-price mono" }, formatVND(cartLineTotal(item))),
      el("button", { class: "btn btn--danger btn--sm", onclick: async () => { await removeCartItem(item.id); render(); refreshCartCount(); } }, "Xoá"),
    ]);
    return row;
  }
  ...
  async function render() {
    ...
    list.innerHTML = "";
    if (!items.length) {
      list.innerHTML = `<div class="empty-state"><h3>Giỏ hàng trống</h3><p>Hãy khám phá sản phẩm và thêm vào giỏ.</p><a href="/showcase/pcstore-single/" class="btn btn--primary" style="margin-top:14px;">Tiếp tục mua sắm</a></div>`;
      document.getElementById("checkout-btn").setAttribute("style", "pointer-events:none; opacity:.4;");
    } else {
      for (const it of items) list.appendChild(cartRow(it));
      document.getElementById("checkout-btn").removeAttribute("style");
    }
    const subtotal = items.reduce((s, it) => s + cartLineTotal(it), 0);
    const ship = items.length === 0 ? 0 : (subtotal >= FREE_SHIP_THRESHOLD ? 0 : SHIPPING_FEE);
    document.getElementById("sum-subtotal").textContent = formatVND(subtotal);
    document.getElementById("sum-ship").textContent = ship === 0 ? "Miễn phí" : formatVND(ship);
    document.getElementById("sum-total").textContent = formatVND(subtotal + ship);
    refreshCartCount();
  }

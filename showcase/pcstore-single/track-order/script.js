...
let user;
  ...
  function renderTracking(order) {
    ...
    if (order.status === "cancelled") {
      resultBox.appendChild(el("div", { class: "order-card" }, [
        el("div", { class: "order-card-head" }, [
          el("div", { class: "mono", style: "font-weight:700;" }, order.code),
          el("span", { class: "badge badge--cancelled" }, "Đã huỷ"),
        ]),
        el("p", { style: "color:var(--muted);" }, "Đơn hàng này đã bị huỷ."),
      ]));
      return;
    }

    const currentIdx = STATUS_STEPS.indexOf(order.status);
    const steps = el("div", { class: "track-steps" }, STATUS_STEPS.map((s, i) =>
      el("div", { class: "track-step" + (i <= currentIdx ? " done" : "") }, [
        el("div", { class: "track-dot" }, String(i + 1)),
        el("div", { class: "track-label" }, STATUS_LABEL[s]),
      ])
    ));
    ...
    const card = el("div", { class: "order-card" }, [
      el("div", { class: "order-card-head" }, [
        el("div", {}, [
          el("div", { class: "mono", style: "font-weight:700; font-size:1.1rem;" }, order.code),
          el("div", { style: "color:var(--muted); font-size:.85rem;" }, `Đặt lúc ${new Date(order.created_at).toLocaleString("vi-VN")}`),
        ]),
        el("span", { class: `badge badge--${order.status}` }, STATUS_LABEL[order.status]),
      ]),
      steps,
      el("table", { class: "spec-table" }, [
        el("tr", {}, [el("td", {}, "Người nhận"), el("td", {}, order.receiver_name)]),
        el("tr", {}, [el("td", {}, "Điện thoại"), el("td", {}, order.receiver_phone)]),
        el("tr", {}, [el("td", {}, "Địa chỉ"), el("td", {}, order.receiver_address)]),
        el("tr", {}, [el("td", {}, "Thanh toán"), el("td", {}, order.payment_method.toUpperCase() + " — " + (order.payment_status === "paid" ? "Đã thanh toán" : "Chưa thanh toán"))]),
        el("tr", {}, [el("td", {}, "Tổng tiền"), el("td", { class: "mono", style: "color:var(--cyan); font-weight:700;" }, formatVND(order.total))]),
      ]),
    ]);
  ...
  async function track(code) {
    if (!code) return;
    resultBox.innerHTML = `<div class="empty-state">Đang tra cứu...</div>`;
    try {
      const order = await fetchOrderByCode(code, user.id);
      renderTracking(order);
    } catch (e) {
      resultBox.innerHTML = `<div class="empty-state"><h3>Không tìm thấy đơn hàng</h3><p>Kiểm tra lại mã đơn hàng.</p></div>`;
    }
  }

  document.getElementById("track-btn").addEventListener("click", () => track(document.getElementById("code-input").value.trim()));
  document.getElementById("code-input").addEventListener("keydown", (e) => { if (e.key === "Enter") track(e.target.value.trim()); });

  async function init() {
    user = await requireAuth(`login.html?redirect=${encodeURIComponent(location.pathname + location.search)}`);
    if (!user) return;
    mountHeaderAuthState({ userSlotEl: document.getElementById("user-slot") });
    const code = new URLSearchParams(location.search).get("code");
    if (code) { document.getElementById("code-input").value = code; track(code); }
  }
  init();

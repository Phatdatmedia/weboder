...
async function requireAuth(redirect = "/showcase/pcstore-single/admin/login.html") {
  ...
  if (userSlotEl) {
    if (user) {
      ...
      document.getElementById("btn-logout")?.addEventListener("click", async () => {
        await signOut();
        window.location.href = location.pathname.includes('/admin') ? "/showcase/pcstore-single/login/" : "/showcase/pcstore-single/admin/login.html";
      });
    } else {
      userSlotEl.innerHTML = `<a href="/showcase/pcstore-single/admin/login.html" class="nav-link">Đăng nhập</a>`;
    }
  }
  return user;
}
...
let staffId;
  ...
  function orderRow(o) {
    const statusSelect = el("select", { class: "status-select" },
      Object.entries(STATUS_LABEL).map(([val, label]) => {
        ...
        opt.value = val; opt.textContent = label; if (val === o.status) opt.selected = true;
        return opt;
      })
    );
    statusSelect.addEventListener("change", async () => {
      try { await adminUpdateOrderStatus(o.id, statusSelect.value, staffId); toast("Đã cập nhật trạng thái", "success"); }
      catch (e) { toast(e.message, "error"); }
    });

    const paySelect = el("select", { class: "status-select" }, [
      el("option", { value: "unpaid", selected: o.payment_status === "unpaid" ? "true" : null }, "Chưa thanh toán"),
      el("option", { value: "paid", selected: o.payment_status === "paid" ? "true" : null }, "Đã thanh toán"),
      el("option", { value: "refunded", selected: o.payment_status === "refunded" ? "true" : null }, "Đã hoàn tiền"),
    ]);
    paySelect.addEventListener("change", async () => {
      try { await adminUpdatePaymentStatus(o.id, paySelect.value); toast("Đã cập nhật thanh toán", "success"); }
      catch (e) { toast(e.message, "error"); }
    });

    return el("tr", {}, [
      el("td", { class: "mono" }, o.code),
      el("td", {}, o.profiles?.full_name || o.receiver_name),
      el("td", { class: "mono" }, o.receiver_phone),
      el("td", { class: "mono" }, formatVND(o.total)),
      el("td", {}, paySelect),
      el("td", {}, statusSelect),
      el("td", {}, new Date(o.created_at).toLocaleDateString("vi-VN")),
      el("td", {}, el("button", { class: "btn btn--sm", onclick: () => showDetail(o) }, "Chi tiết")),
    ]);
  }

  function showDetail(o) {
    const items = o.order_items.map(it => `${it.product_name} × ${it.quantity} — ${formatVND(it.unit_price * it.quantity)}`).join("\n");
    alert(`Đơn hàng ${o.code}\n\nNgười nhận: ${o.receiver_name}\nSĐT: ${o.receiver_phone}\nĐịa chỉ: ${o.receiver_address}\nGhi chú: ${o.note || "—"}\n\nSản phẩm:\n${items}\n\nTạm tính: ${formatVND(o.subtotal)}\nPhí ship: ${formatVND(o.shipping_fee)}\nTổng: ${formatVND(o.total)}`);
  }
  ...
  async function loadOrders() {
    ...
    const orders = await adminFetchOrders({ status: status || undefined });
    tbody.innerHTML = "";
    if (!orders.length) { tbody.innerHTML = `<tr><td colspan="8">Không có đơn hàng nào.</td></tr>`; return; }
    for (const o of orders) tbody.appendChild(orderRow(o));
  }

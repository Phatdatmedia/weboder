...
// ---- page logic ----
function profileRow(p, isAdmin) {
    const roleSelect = el("select", { class: "status-select" }, [
      el("option", { value: "customer", selected: p.role === "customer" ? "true" : null }, "Khách hàng"),
      el("option", { value: "staff", selected: p.role === "staff" ? "true" : null }, "Nhân viên"),
      el("option", { value: "admin", selected: p.role === "admin" ? "true" : null }, "Quản trị viên"),
    ]);
    if (!isAdmin) roleSelect.disabled = true;
    roleSelect.addEventListener("change", async () => {
      try { await updateUserRole(p.id, roleSelect.value); toast("Đã cập nhật vai trò", "success"); }
      catch (e) { toast("Chỉ admin mới có quyền này: " + e.message, "error"); }
    });
    return el("tr", {}, [
      el("td", {}, p.full_name || "—"),
      el("td", { class: "mono" }, p.phone || "—"),
      el("td", {}, new Date(p.created_at).toLocaleDateString("vi-VN")),
      el("td", {}, roleSelect),
    ]);
  }
  ...
  async function init() {
    ...
    tbody.innerHTML = "";
    for (const p of profiles) tbody.appendChild(profileRow(p, isAdmin));
  }
  init();

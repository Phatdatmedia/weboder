...
const grid = document.getElementById("product-grid");
  ...
  let state = { category: params.get("cat") || "", search: params.get("q") || "", sort: "newest" };
  ...
  async function renderCategories() {
    ...
    catBar.innerHTML = "";
    const allChip = el("a", { class: "chip" + (state.category ? "" : " active"), href: "/showcase/pcstore-single/" }, "Tất cả");
    catBar.appendChild(allChip);
    for (const c of cats) {
      const chip = el("a", {
        class: "chip" + (state.category === c.slug ? " active" : ""),
        href: `index.html?cat=${c.slug}`,
      }, c.name);
      catBar.appendChild(chip);
    }
    if (state.category) {
      const found = cats.find(c => c.slug === state.category);
      catalogTitle.textContent = found ? found.name : "Sản phẩm";
    }
  }
  ...
  function productCard(p) {
    ...
    const card = el("div", { class: "product-card" }, [
      el("a", { href: `product.html?slug=${p.slug}` }, [
        el("div", { class: "product-thumb" }, [
          el("img", { src: p.image_url || "https://placehold.co/400x400/191C26/57E8CB?text=PC.STORE", alt: p.name }),
        ]),
      ]),
      el("div", { class: "product-body" }, [
        el("div", { class: "product-cat" }, p.categories?.name || ""),
        el("a", { href: `product.html?slug=${p.slug}` }, el("div", { class: "product-name" }, p.name)),
        el("div", { class: "product-specs" }, specs.map(([k, v]) => el("span", {}, `${k}: ${v}`))),
        el("div", { class: "product-price-row" }, [
          el("span", { class: "product-price mono" }, formatVND(price)),
          p.sale_price ? el("span", { class: "product-price--old mono" }, formatVND(p.price)) : null,
        ]),
        el("div", { class: "stock-tag" + (p.stock <= 0 ? " stock-tag--out" : "") }, p.stock > 0 ? `Còn ${p.stock} sp` : "Hết hàng"),
        el("button", {
          class: "btn btn--primary btn--full btn--sm",
          disabled: p.stock <= 0 ? "true" : null,
          onclick: async () => {
            const user = await getCurrentUser();
            if (!user) { window.location.href = "/showcase/pcstore-single/login/"; return; }
            try {
  ...
  async function renderProducts() {
    ...
    try {
      const products = await fetchProducts({ categorySlug: state.category, search: state.search, sort: state.sort });
      document.getElementById("stat-products").textContent = products.length;
      grid.innerHTML = "";
      if (!products.length) {
        grid.innerHTML = `<div class="empty-state"><h3>Không tìm thấy sản phẩm</h3><p>Thử từ khoá khác hoặc chọn danh mục khác.</p></div>`;
        return;
      }
      for (const p of products) grid.appendChild(productCard(p));
    } catch (e) {
      grid.innerHTML = `<div class="empty-state"><h3>Chưa kết nối được Supabase</h3><p>Kiểm tra js/config.js đã điền đúng SUPABASE_URL và ANON_KEY chưa.</p></div>`;
      console.error(e);
    }
  }
  ...
  searchInput.addEventListener("input", () => {
    ...
    searchTimer = setTimeout(() => { state.search = searchInput.value.trim(); renderProducts(); }, 350);
  });

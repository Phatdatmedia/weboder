...
let user, cartItems = [];
  ...
  async function render() {
    ...
    miniCart.innerHTML = "";
    if (!cartItems.length) {
      document.querySelector(".section").innerHTML = `<div class="empty-state"><h3>Giỏ hàng trống</h3><p>Hãy thêm sản phẩm trước khi thanh toán.</p><a href="/showcase/pcstore-single/" class="btn btn--primary" style="margin-top:14px;">Về trang chủ</a></div>`;
      return;
    }
    for (const it of cartItems) {
      miniCart.appendChild(el("div", { style: "display:flex; justify-content:space-between; font-size:.85rem; margin-bottom:8px;" }, [
        el("span", {}, `${it.products.name} × ${it.quantity}`),
        el("span", { class: "mono" }, formatVND(cartLineTotal(it))),
      ]));
    }
    const subtotal = cartItems.reduce((s, it) => s + cartLineTotal(it), 0);
    const ship = subtotal >= FREE_SHIP_THRESHOLD ? 0 : SHIPPING_FEE;
  ...
  document.getElementById("place-order-btn").addEventListener("click", async () => {
    ...
    if (!name || !phone || !address) { toast("Vui lòng điền đầy đủ thông tin giao hàng", "error"); return; }

    const btn = document.getElementById("place-order-btn");
    btn.disabled = true; btn.textContent = "Đang xử lý...";
    try {
      const order = await placeOrder({
        userId: user.id, cartItems, paymentMethod, note,
        receiver: { name, phone, address },
      });
      window.location.href = `/showcase/pcstore-single/order-success/?code=${order.code}`;
    } catch (e) {

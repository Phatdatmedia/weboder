...
let user;
  ...
  document.getElementById("save-profile-btn").addEventListener("click", async () => {
    ...
    else toast("Đã lưu thông tin tài khoản", "success");
  });
  ...
  function orderCard(o) {
    const card = el("div", { class: "order-card" }, [
      el("div", { class: "order-card-head" }, [
        el("div", {}, [
          el("div", { class: "mono", style: "font-weight:700;" }, o.code),
          el("div", { style: "color:var(--muted); font-size:.8rem;" }, new Date(o.created_at).toLocaleString("vi-VN")),
        ]),
        el("span", { class: `badge badge--${o.status}` }, STATUS_LABEL[o.status] || o.status),
      ]),
      el("div", { style: "color:var(--muted); font-size:.85rem; margin-bottom:10px;" },
        o.order_items.map((it) => `${it.product_name} × ${it.quantity}`).join(", ")),
      el("div", { style: "display:flex; justify-content:space-between; align-items:center;" }, [
        el("span", { class: "mono", style: "font-weight:700; color:var(--cyan);" }, formatVND(o.total)),
        el("div", { style: "display:flex; gap:8px;" }, [
          el("a", { href: `track-order.html?code=${o.code}`, class: "btn btn--sm" }, "Theo dõi"),
          o.status === "pending" ? el("button", {
            class: "btn btn--danger btn--sm",
            onclick: async () => { await cancelMyOrder(o.id); toast("Đã huỷ đơn hàng", "success"); loadOrders(); },
          }, "Huỷ đơn") : null,
        ]),
      ]),
    ]);
  ...
  async function loadOrders() {
    ...
    list.innerHTML = "";
    if (!orders.length) { list.innerHTML = `<div class="empty-state"><h3>Chưa có đơn hàng nào</h3></div>`; return; }
